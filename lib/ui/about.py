...
class AboutBox(wx.Dialog):
    ...
    def __init__(self, parent=None, id=-1, title=None,
        ...
        '''Note that if title is None and the parent has the attr
        "Title", the title will be "About "+getattr(parent, "Title")'''
    ...
    def SetCenterIcon(self, file_name, file_type=wx.BITMAP_TYPE_ANY,
                      size=wx.DefaultSize):
        ...
        sizer_item = self._sizer.Insert(0, img_box, 0,
    ...
    def SetTitleIcon(self, file_name, file_type=wx.BITMAP_TYPE_ANY):
        ...
        It will not set the center icon showed.If needed,
        you can call "SetCenterIcon()"'''
        icon = wx.Icon(file_name, file_type)
        return super(AboutBox, self).SetIcon(icon)

    def SetName(self, name, font_size=15):
        '''SetName(str name) -> SizerItem
        Set the name of your program. Call after "SetVersion" or the version
        will not work. Note that it does not change the title of the program
        and the displayed name is "name"+"version". You should never change
        the name later otherwise the center style will disappear.'''
        name = name.strip()
        self._name = name
        self._has_name = True
        index = 0
        ...
        sizer_item = self._sizer.Insert(
            index, name_box, 0, wx.ALIGN_CENTER | wx.FIXED_MINSIZE | wx.EXPAND)
    ...
    def SetVersion(self, version):
        ...
        Set the version of the program. Call this before "SetName"
        otherwise it will not work. return version.strip()'''
    ...
    def SetCopyright(self, copyright, font_size=3):
        ...
        copyright = copyright.replace(
            '(c)',
            '</font><font size="%s">&copy;</font><font size="%s">' %
        ...
        if self._has_name is True:
            index += 1
        sizer_item = self._sizer.Insert(index, htm, 0,
    ...
    def SetDescription(self, description):
        ...
        sizer_item = self._sizer.Insert(
            index, desc, 0,
            wx.ALIGN_CENTER | wx.FIXED_MINSIZE | wx.ALL, self.BORDER)
    ...
    def SetWebsite(self, link, label=None):
        ...
        linker = HyperlinkCtrl(self, label=label, url=link,
        ...
        if self._description:
            index += 1
        sizer_item = self._sizer.Insert(index, linker, 0, wx.ALIGN_CENTER)
    ...
    def AddDetail(self, label, context, force=False):
        ...
        Add a button with given label(">>" and "<<" at the end means
        show or hide), a static line, then add a statictext of given context.
        set force=True if you want to pass an object(even a sizer)
        as context'''
    ...
    def ShowModal(self):
        if not self.FindWindowById(wx.ID_OK):
            ...
            self._sizer.Add(btn_ok, 0,
                            wx.ALL | wx.ALIGN_RIGHT | wx.ALIGN_BOTTOM,
                            self.BORDER)
        self._sizer.Layout()
        self.Fit()
